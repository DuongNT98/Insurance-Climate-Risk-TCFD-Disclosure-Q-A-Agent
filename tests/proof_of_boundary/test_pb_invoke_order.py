...
class TestInvokeOrder:
    ...
    def test_call_order_for_every_node(self, monkeypatch):
        ...
        # Local wheel/site-packages mirror can be a stale stub that predates
        # emit_trace_event on base_node — this is a known local-only artifact
        # covered by the framework's CI verification reference docs (Proof-of-
        # Boundary PB-6); CI's agenticstar-agentcore==1.0.0 wheel is the gate of record.
        if not hasattr(base_node_module, "emit_trace_event"):
            pytest.skip("local framework mirror lacks emit_trace_event - stale stub; CI wheel is gate of record")
