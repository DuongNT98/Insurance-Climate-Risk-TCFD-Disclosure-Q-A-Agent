...
# This template uses neither, so this compiles with no checkpointer.
...
agent.provision_secrets(secrets_factory(namespace="ins", agent_name="ins-c2-013"))
...
async def invoke(req: InvokeRequest, request: Request) -> Any:
    ...
    # before auth); this is the framework's documented entry-point auth exception.
...
def health() -> dict[str, str]:
    return {"status": "ok", "agent": "ins-c2-013"}
